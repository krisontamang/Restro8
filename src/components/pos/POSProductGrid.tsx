...
export function POSProductGrid({ items, onItemClick, searchQuery = '' }: POSProductGridProps) {
  ...
  return <div className="pos-menu-grid">{items.map(item => {
    ...
    return <button type="button" className="pos-menu-item food-card" key={item.id} disabled={soldOut} aria-label={soldOut ? `${item.name} is sold out` : `Add ${item.name} to ticket`} onClick={() => onItemClick(item)}>
      <div className="pos-dish-image-wrapper"><FoodImage item={item} /><span className="food-prep"><Clock size={11} /> {item.prepTimeMinutes} min</span>{item.tags.includes('veg') && <span className="food-veg">Vegetarian</span>}{soldOut && <span className="food-soldout">Sold out</span>}</div>
