...
export const SuppliersView: React.FC = () => {
  ...
  const totalToPay = suppliers.reduce((acc, s) => acc + (s.dueAmount > 0 ? s.dueAmount : 0), 0);
  const totalToReceive = suppliers.reduce((acc, s) => acc + (s.dueAmount < 0 ? Math.abs(s.dueAmount) : 0), 0);
  const netToReceive = totalToReceive - totalToPay;
  ...
  const handleExportCSV = () => {
    const headers = ['SN,Supplier,Phone Number,DOB,Due Amount,Email'];
    const rows = suppliers.map(
      (s) => `${s.sn},"${s.name}","${s.phone}","${s.dob}",${s.dueAmount},"${s.email}"`
    );
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1440px', margin: '0 auto' }}>
            ...
            {isOptionsMenuOpen && (
              <>
                ...
                >
                  ...
                  <button
                    onClick={() => {
                      notifyToast('Column Settings', 'Columns visible: Supplier, Phone Number, DOB, Due Amount, Email', 'info');
                      setIsOptionsMenuOpen(false);
                    }}
                    style={{
                      display: 'flex',
      ...
      {/* Top 3 KPI Cards matching Screenshot 4 */}
      <div
        style={{
          display: 'grid',
      ...
      >
        ...
        >
          ...
          <div style={{ fontSize: '1.75rem', fontWeight: 900, color: 'var(--color-foreground)', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            Rs {netToReceive.toLocaleString()}
          </div>
        </div>
      </div>
      ...
      >
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', minWidth: '940px' }}>
            <thead>
              ...
              >
                ...
                <th style={{ padding: '14px 18px', fontSize: '0.84rem', fontWeight: 700 }}>Phone Number</th>
                <th style={{ padding: '14px 18px', fontSize: '0.84rem', fontWeight: 700 }}>DOB</th>
                <th style={{ padding: '14px 18px', fontSize: '0.84rem', fontWeight: 700 }}>Due Amount</th>
                <th style={{ padding: '14px 18px', fontSize: '0.84rem', fontWeight: 700 }}>Email</th>
              </tr>
            </thead>
              ...
              <tbody>
                {filtered.map((s) => (
                  <tr key={s.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    <td style={{ padding: '14px 18px', fontSize: '0.86rem', color: 'var(--color-muted-foreground)' }}>{s.sn}</td>
                    <td style={{ padding: '14px 18px', fontSize: '0.88rem', fontWeight: 700 }}>{s.name}</td>
                    <td style={{ padding: '14px 18px', fontSize: '0.86rem' }}>{s.phone}</td>
                    <td style={{ padding: '14px 18px', fontSize: '0.86rem', color: 'var(--color-muted-foreground)' }}>{s.dob}</td>
                    <td style={{ padding: '14px 18px', fontSize: '0.88rem', fontWeight: 800 }}>Rs {s.dueAmount.toLocaleString()}</td>
                    <td style={{ padding: '14px 18px', fontSize: '0.86rem', color: 'var(--color-muted-foreground)' }}>{s.email}</td>
                  </tr>
                ))}
              </tbody>
            )}
        ...
        {filtered.length === 0 && (
          ...
          >
            ...
            >
              Create a new supplier or import a new data.
            </p>
      ...
      {/* Selected Row Counter matching Screenshot 4 */}
      <div
        style={{
          marginTop: '12px',
          fontSize: '0.82rem',
      ...
      {isAddModalOpen && (
        ...
        >
          ...
          >
            ...
            >
              <h3 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 800 }}>Add New Supplier</h3>
              <button
            ...
            <form onSubmit={handleCreate} style={{ padding: '24px' }}>
              <div style={{ marginBottom: '14px' }}>
                ...
                <input
                  type="text"
                  placeholder="e.g. Ilam Agro Tea, Valley Dairy Products"
                  value={supplierName}
