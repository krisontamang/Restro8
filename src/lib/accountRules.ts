...
export function resolvePublicRoute(path: string, query = ''): PublicRoute {
  const normalized = path.toLowerCase().replace(/\/$/, '') || '/';
  const routes: Record<string, PublicRoute> = { '/':'landing', '/np':'landing', '/login':'login', '/signin':'login', '/auth':'login', '/signup':'signup', '/forgot-password':'forgot', '/reset-password':'reset', '/auth/callback':'callback', '/app':'account', '/account':'account', '/workspace':'account', '/dashboard':'account', '/pos':'account', '/demo':'demo', '/menu':'menu' };
  if (normalized === '/') {
